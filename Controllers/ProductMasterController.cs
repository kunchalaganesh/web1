...
namespace JewelleryWebApplication.Controllers
{
    ...
    public class ProductMasterController : ControllerBase
    {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (HttpContext.Request.Form.Files.Count <= 5)
                {
                    ...
                    for (int i = 1; i <= products.Quantity; i++)
                    {
                        ...
                        int BoxId = Convert.ToInt32(HttpContext.Request.Form["BoxId"]);
                       int PartyTypeId = Convert.ToInt32(HttpContext.Request.Form["PartyTypeId"]);
                        var partydata = _partyMasterRepository.All().Where(x => x.Id == PartyTypeId).FirstOrDefault();
                        var boxdata = _boxmasterRepository.All().Where(x => x.Id == BoxId).FirstOrDefault();
                        ...
                        tblProductsDetails.tidValue = product.TID;
                   //     tblProductsDetails.createdBy = "";
                        ...
                        tblProductsDetails.product = product.ItemType;
                        ...
                        tblProductsDetails.stoneweight = product.StoneWeight;
        ...
        public async Task<IActionResult> BulkUpdateProduct(List<tblProduct> list)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                foreach (var model in list)
                {
                    ...
                    var productdetails = _productDetailsRepository.All().Where(x => x.itemCode.ToUpper() == products.ItemCode.ToUpper()).FirstOrDefault();
                    if (products != null)
                    {
                        ...
                        productdetails.tidValue = product.TID;

                        productdetails.category = product.Category_Name;
                        //itemtype=product
                        productdetails.barcodeNumber = product.BarcodeNumber;
                        productdetails.product = product.ItemType;
                        productdetails.purity = product.purity;
                        productdetails.itemCode = product.ItemCode;
                        productdetails.box = boxdata.BoxName;
                        productdetails.grossWeight = product.grosswt;
                        productdetails.netWeight = product.NetWt;
                        productdetails.stoneweight = product.StoneWeight;
                        productdetails.makinggm = product.Making_per_gram;
                        productdetails.makingper = product.Making_Percentage;
                        productdetails.fixedamount = product.Making_Fixed_Amt;
                        productdetails.fixedwastage = product.Making_Fixed_Wastage;
                        productdetails.stoneamount = product.StoneAmount;
                        productdetails.mrp = product.MRP.ToString();
                        productdetails.hudicode = product.HUIDCode;
                        productdetails.partycode = partydata.supplier_code;
                        productdetails.updatedDate = product.LastUpdated.ToString();
                    
                        await _productDetailsRepository.UpdateAsync(productdetails, productdetails.Id);
                        await _productrepository.UpdateAsync(product, product.Id);
        ...
        [HttpPost("InsertBulkProduct")]
        public async Task<IActionResult> InsertBulkProduct()
        {
            int CollectionId = Convert.ToInt32(HttpContext.Request.Form["CollectionId"]);
            ...
            if (ModelState.IsValid)
            {
                List<tblRFID> tblrfid = new List<tblRFID>();
                List<tblProductsDetails> list = new List<tblProductsDetails>();
                List<tblProduct> li = new List<tblProduct>();
                tblProduct products = new tblProduct();
                ...
                if (HttpContext.Request.Form.Files.Count <= 5)
                {
                    ...
                    for (int i = 1; i <= products.Quantity; i++)
                    {
                        ...
                        tblProductsDetails.tagtransaction = "";
                        tblProductsDetails.status = "";
                        tblRFID.TID = tblProductsDetails.tidValue;
                        tblRFID.BarcodeNumber = tblProductsDetails.barcodeNumber;
                        li.Add(product);
                        list.Add(tblProductsDetails);
                        tblrfid.Add(tblRFID);
                    }
                    await _productDetailsRepository.BulkInsertAsync(list);
                    await _productrepository.BulkInsertAsync(li);
                    await _tblSecretRepository.BulkInsertAsync(tblrfid);
                    return Ok(new { status = "Success", data = li });
                }
                else
                {
        ...
        [HttpPost("BulkEditProduct")]
        public async Task<IActionResult> BulkEditProduct(List<tblProduct> list)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                foreach (var model in list)
                {
                    ...
                    var products = _productrepository.All().Where(x => x.Id == model.Id).FirstOrDefault();
                    var productdetails = _productDetailsRepository.All().Where(x => x.itemCode.ToUpper() == products.ItemCode.ToUpper()).FirstOrDefault();
                    var rfiddata = _tblSecretRepository.All().Where(x => x.Id == productdetails.Id).FirstOrDefault();
                    if (products != null)
                    {
                        var boxdata = _boxmasterRepository.All().Where(x => x.Id == model.BoxId).FirstOrDefault();
                        var partydata = _partyMasterRepository.All().Where(x => x.Id == model.PartyTypeId).FirstOrDefault();
                        ...
                        productdetails.partycode = partydata.supplier_code;
                        productdetails.updatedDate = product.LastUpdated.ToString();
                        rfiddata.TID= product.TID;
                        rfiddata.BarcodeNumber = product.BarcodeNumber;
                        await _productDetailsRepository.UpdateAsync(productdetails, productdetails.Id);
                        await _productrepository.UpdateAsync(product, product.Id);
                        await _tblSecretRepository.UpdateAsync(rfiddata, rfiddata.Id);
                    }
